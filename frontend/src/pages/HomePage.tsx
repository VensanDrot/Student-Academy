...
const HomePage = () => {
  ...
  useEffect(() => {
    if (retrievedCourses?.courses && !isLoadingCourses) {
      setCourses((prev) => [...prev, ...retrievedCourses?.courses]);
      setIsLoadMore(retrievedCourses?.courses.length === 6); // Stop loading if fewer items are returned
  ...
  return (
    <div className="w-full h-auto px-6 flex flex-col gap-4">
      ...
      <div className="w-full h-fit">
        <DisplayCategories active={active} setActive={setActive} />
      </div>
        ...
        <div className="flex flex-wrap gap-8 gap-y-6 w-fit">
          {courses?.map((course: CourseOrgAll, index) => (
            <CoursePreviewComponent
              ...
              onDetails={() => {
                navigate(`coursePreview?id=${course?.id}&name=${course?.name}`);
              }}
              onBuy={
                course?.purchased
                  ? undefined
